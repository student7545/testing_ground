...
L({
  ...
  steps: [
    ...
    { d: 'SW1', t: 'Practise leaving privileged mode two different ways, and a simulated reboot.', c: ['disable', 'enable', 'cisco123', 'logout', 'enable', 'cisco123', 'reload'], note: '<code>disable</code> steps down from # to >. <code>logout</code> ends the session entirely. <code>reload</code> reboots \u2014 IOS warns you first if the running config was never saved. Here the reload is simulated, so your work survives.' },
...
L({
  ...
  steps: [
    ...
    { d: 'R1', t: 'Remove the host route and watch the path revert.', c: ['configure terminal', 'no ip route 10.0.3.10 255.255.255.255 10.0.13.3', 'end', 'show ip route', 'traceroute 10.0.3.10'], note: 'Traffic falls back to the /24 via R2 \u2014 three hops now. This is longest prefix match in action.' },
